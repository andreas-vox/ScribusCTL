...
*/
    ...
    if ((itLen != 0)) // || (NextBox != 0))
    {
        ...
        for (int a = firstInFrame(); a < itLen; ++a)
        {
            ...
            if (current.charsInLine == 0)
            {
                current.glyphRuns.last().setFlag(ScLayout_StartOfLine);
                kernVal = 0;
            }
            else
            {
                kernVal = 0; // chs * charStyle.tracking() / 10000.0;
                current.glyphRuns.last().clearFlag(ScLayout_StartOfLine);
            }
            //			glyphs->yadvance = 0;
            layoutGlyphs(chstr, current.glyphRuns.last());
			...
			GlyphLayout *firstGlyph;
			firstGlyph = &(currentRun.glyphs().first());
			GlyphLayout *lastGlyph;
			lastGlyph = &(currentRun.glyphs().last());
...
//					wide = current.xPos - firstGlyph.xadvance ;
            ...
            else
            {
                ...
                if (outs || current.xPos >= current.mustLineEnd-5)
                {
                    if (current.charsInLine == 0)
                    {
