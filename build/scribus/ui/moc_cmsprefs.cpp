/****************************************************************************
** Meta object code from reading C++ file 'cmsprefs.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.4.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../scribus/ui/cmsprefs.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'cmsprefs.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.4.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_CMSPrefs_t {
    QByteArrayData data[6];
    char stringdata[43];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_CMSPrefs_t, stringdata) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_CMSPrefs_t qt_meta_stringdata_CMSPrefs = {
    {
QT_MOC_LITERAL(0, 0, 8), // "CMSPrefs"
QT_MOC_LITERAL(1, 9, 5), // "cmsOn"
QT_MOC_LITERAL(2, 15, 0), // ""
QT_MOC_LITERAL(3, 16, 9), // "slotCMSon"
QT_MOC_LITERAL(4, 26, 6), // "active"
QT_MOC_LITERAL(5, 33, 9) // "setValues"

    },
    "CMSPrefs\0cmsOn\0\0slotCMSon\0active\0"
    "setValues"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_CMSPrefs[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       1,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    1,   34,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       3,    1,   37,    2, 0x0a /* Public */,
       3,    0,   40,    2, 0x2a /* Public | MethodCloned */,
       5,    0,   41,    2, 0x0a /* Public */,

 // signals: parameters
    QMetaType::Void, QMetaType::Bool,    2,

 // slots: parameters
    QMetaType::Void, QMetaType::Bool,    4,
    QMetaType::Void,
    QMetaType::Void,

       0        // eod
};

void CMSPrefs::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        CMSPrefs *_t = static_cast<CMSPrefs *>(_o);
        switch (_id) {
        case 0: _t->cmsOn((*reinterpret_cast< bool(*)>(_a[1]))); break;
        case 1: _t->slotCMSon((*reinterpret_cast< bool(*)>(_a[1]))); break;
        case 2: _t->slotCMSon(); break;
        case 3: _t->setValues(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (CMSPrefs::*_t)(bool );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&CMSPrefs::cmsOn)) {
                *result = 0;
            }
        }
    }
}

const QMetaObject CMSPrefs::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_CMSPrefs.data,
      qt_meta_data_CMSPrefs,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *CMSPrefs::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *CMSPrefs::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_CMSPrefs.stringdata))
        return static_cast<void*>(const_cast< CMSPrefs*>(this));
    if (!strcmp(_clname, "Ui::CMSPrefsBase"))
        return static_cast< Ui::CMSPrefsBase*>(const_cast< CMSPrefs*>(this));
    return QWidget::qt_metacast(_clname);
}

int CMSPrefs::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 4)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 4;
    }
    return _id;
}

// SIGNAL 0
void CMSPrefs::cmsOn(bool _t1)
{
    void *_a[] = { Q_NULLPTR, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}
QT_END_MOC_NAMESPACE
